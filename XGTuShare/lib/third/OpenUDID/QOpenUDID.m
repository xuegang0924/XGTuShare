...
+ (NSString*) value {
    ...
    // 如果已经存在，直接使用缓存中的udid
    if (openUdid && openUdid.length > 0) return openUdid;
    
    @try {
        KeychainItemWrapper* wrapper = [[KeychainItemWrapper alloc] initWithIdentifier:@"kQiYiOpenUDID" accessGroup:nil];
        
        // 保留引用计数
        openUdid = [[wrapper objectForKey:(id)kSecAttrAccount] retain];
        if (openUdid.length == 0) {
            [openUdid release];
            openUdid = [[QOpenUDID valueWithError:nil] retain];
            [wrapper setObject:openUdid forKey:(id)kSecAttrAccount];
        }
        [wrapper release];
    }
    @catch (NSException *exception) {
        NSLog(@"get openudid failed");
    }
    @finally {
    }
    if (openUdid.length<1) {
        openUdid = @"";
    }
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [QOpenUDID _generateFreshOpenUDID];
        } else {

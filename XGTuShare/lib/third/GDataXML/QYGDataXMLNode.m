...
#import "QYGDataXMLNode.h"

@class NSArray, NSDictionary, NSError, NSString, NSURL;
@class QYGDataXMLElement, QYGDataXMLDocument;
...
// QYGDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
    ...
    NSString *localName = [QYGDataXMLNode localNameForName:name];
...
@interface QYGDataXMLNode (PrivateMethods)
...
- (QYGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
+ (QYGDataXMLElement *)elementWithName:(NSString *)name {
...
+ (QYGDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
...
+ (QYGDataXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
    ...
    if (theXMLNode->type == XML_ELEMENT_NODE) {
        theClass = [QYGDataXMLElement class];
    } else {
        theClass = [QYGDataXMLNode class];
    }
...
#if DEBUG
    NSCAssert(chars != NULL, @"QYGDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
    ...
    if (xmlNode_ != NULL) {
        ...
        while (currChild != NULL) {
            QYGDataXMLNode *node = [QYGDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (QYGDataXMLNode *)childAtIndex:(unsigned)index {
...
- (QYGDataXMLNodeKind)kind {
    if (xmlNode_ != NULL) {
        xmlElementType nodeType = xmlNode_->type;
        switch (nodeType) {
            case XML_ELEMENT_NODE:         return QYGDataXMLElementKind;
            case XML_ATTRIBUTE_NODE:       return QYGDataXMLAttributeKind;
            case XML_TEXT_NODE:            return QYGDataXMLTextKind;
            case XML_CDATA_SECTION_NODE:   return QYGDataXMLTextKind;
            case XML_ENTITY_REF_NODE:      return QYGDataXMLEntityDeclarationKind;
            case XML_ENTITY_NODE:          return QYGDataXMLEntityDeclarationKind;
            case XML_PI_NODE:              return QYGDataXMLProcessingInstructionKind;
            case XML_COMMENT_NODE:         return QYGDataXMLCommentKind;
            case XML_DOCUMENT_NODE:        return QYGDataXMLDocumentKind;
            case XML_DOCUMENT_TYPE_NODE:   return QYGDataXMLDocumentKind;
            case XML_DOCUMENT_FRAG_NODE:   return QYGDataXMLDocumentKind;
            case XML_NOTATION_NODE:        return QYGDataXMLNotationDeclarationKind;
            case XML_HTML_DOCUMENT_NODE:   return QYGDataXMLDocumentKind;
            case XML_DTD_NODE:             return QYGDataXMLDTDKind;
            case XML_ELEMENT_DECL:         return QYGDataXMLElementDeclarationKind;
            case XML_ATTRIBUTE_DECL:       return QYGDataXMLAttributeDeclarationKind;
            case XML_ENTITY_DECL:          return QYGDataXMLEntityDeclarationKind;
            case XML_NAMESPACE_DECL:       return QYGDataXMLNamespaceKind;
            case XML_XINCLUDE_START:       return QYGDataXMLProcessingInstructionKind;
            case XML_XINCLUDE_END:         return QYGDataXMLProcessingInstructionKind;
            case XML_DOCB_DOCUMENT_NODE:   return QYGDataXMLDocumentKind;
        }
    }
    return QYGDataXMLInvalidKind;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
    ...
    // are created from QYGDataXMLElement's initWithXMLString there may not be
    // a document. (We may later decide that we want to stuff the doc used
    // there into a QYGDataXMLDocument and retain it, but we don't do that now.)
    //
        ...
        if (xpathCtx) {
            ...
            if (namespaces) {
                ...
                for (NSString *prefix in namespaces) {
                    ...
                    if (result != 0) {
#if DEBUG
                        NSCAssert1(result == 0, @"QYGDataXMLNode XPath namespace %@ issue",
                                   prefix);
#endif
                    }
                }
                ...
                if (nsNodePtr != NULL) {
                    for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
                        ...
                        if (prefix == NULL) {
                            prefix = (xmlChar*) kQYGDataXMLXPathDefaultNamespacePrefix;
                        }
                        
                        int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
                        if (result != 0) {
#if DEBUG
                            NSCAssert1(result == 0, @"QYGDataXMLNode XPath namespace %s issue",
                                       prefix);
#endif
                        }
                    }
            ...
            if (xpathObj) {
                ...
                if (nodeSet) {
                    ...
                    for (int index = 0; index < nodeSet->nodeNr; index++) {
                        ...
                        QYGDataXMLNode *node = [QYGDataXMLNode nodeBorrowingXMLNode:currNode];
                        if (node) {
                            [array addObject:node];
                        }
                    }
    ...
    if (array == nil && error != nil) {
        *error = [NSError errorWithDomain:@"com.google.QYGDataXML"
                                     code:errorCode
                                 userInfo:errorInfo];
    }
...
- (BOOL)isEqual:(QYGDataXMLNode *)other {
    if (self == other) return YES;
    if (![other isKindOfClass:[QYGDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
    return (NSUInteger) (void *) [QYGDataXMLNode class];
}
...
@implementation QYGDataXMLElement
    ...
    if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
        ...
        while (currNS != NULL) {
            ...
            if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
                QYGDataXMLNode *node = [QYGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)addNamespace:(QYGDataXMLNode *)aNamespace {
...
- (void)addChild:(QYGDataXMLNode *)child {
    if ([child kind] == QYGDataXMLAttributeKind) {
...
- (void)removeChild:(QYGDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
    ...
    if (xmlNode_ != NULL) {
        ...
        for (QYGDataXMLNode *child in children) {
...
- (NSArray *)attributes {
    ...
    if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
        ...
        while (prop != NULL) {
            ...
            QYGDataXMLNode *node = [QYGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
            if (array == nil) {
                array = [NSMutableArray arrayWithObject:node];
            } else {
                [array addObject:node];
...
- (void)addAttribute:(QYGDataXMLNode *)attribute {
...
- (QYGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
    // search the cached attributes list for the QYGDataXMLNode with
    // the underlying xmlAttrPtr
    NSArray *attributes = [self attributes];
    
    for (QYGDataXMLNode *attr in attributes) {
...
- (QYGDataXMLNode *)attributeForName:(NSString *)name {
    ...
    if (xmlNode_ != NULL) {
        ...
        if (attrPtr) {
            QYGDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
            return attr;
        }
    }
    return nil;
...
- (QYGDataXMLNode *)attributeForLocalName:(NSString *)localName
...
@interface QYGDataXMLDocument (PrivateMethods)
- (void)addStringsCacheToDoc;
@end

@implementation QYGDataXMLDocument

- (id)initWithXMLString:(NSString *)str options:(unsigned int)mask error:(NSError **)error {
    
    NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
    QYGDataXMLDocument *doc = [self initWithData:data options:mask error:error];
    return doc;
}
...
- (id)initWithRootElement:(QYGDataXMLElement *)element {
...
- (void)addStringsCacheToDoc {
    ...
    NSCAssert(xmlDoc_ != NULL && xmlDoc_->_private == NULL,
              @"QYGDataXMLDocument cache creation problem");
#endif
...
- (QYGDataXMLElement *)rootElement {
    QYGDataXMLElement *element = nil;
    
    if (xmlDoc_ != NULL) {
        xmlNodePtr rootNode = xmlDocGetRootElement(xmlDoc_);
        if (rootNode) {
            element = [QYGDataXMLElement nodeBorrowingXMLNode:rootNode];
        }
    }
    return element;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
    if (xmlDoc_ != NULL) {
        QYGDataXMLNode *docNode = [QYGDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];

...
- (NSString *) cpuType
{
    switch ([self platformType])
    {
        ...
        case UIDevice5iPhone: return @"Apple A5";
...
//    float usage = 0;
    ...
    if(err == KERN_SUCCESS) {
        ...
        for(unsigned i = 0U; i < _numCPUs; ++i) {
            ...
            //            QYLog(@"Core : %u, Usage: %.2f%%", i, _inUse / _total * 100.f);
...
- (NSUInteger) freeMemoryBytes 
{
    ...
    if (host_statistics(host_port, HOST_VM_INFO, (host_info_t)&vm_stat, &host_size) != KERN_SUCCESS) QYLog(@"Failed to fetch vm statistics");
